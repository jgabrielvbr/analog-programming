...
namespace analog
{
    ...
    {
        const auto insertion_point = std::upper_bound(waves_.begin(), waves_.end(), wave.frequency(), [](double frequency, const analog::wave &candidate)
        {
    ...
    {
        const auto wave = std::lower_bound(waves_.begin(), waves_.end(), frequency, [](const analog::wave &candidate, double frequency)
        {
